...
document.addEventListener('DOMContentLoaded', () => {
  ...
  nav.addEventListener('click', async function (e) {
    ...
    // Hapus semua submenu sebelumnya
    document.querySelectorAll('.children-links').forEach(el => el.remove());

    // Hapus highlight kategori lain
    nav.querySelectorAll('a.side-nav-link').forEach(l => l.classList.remove('active'));
    link.classList.add('active');

    // Tambah submenu jika ada item
    const cat = window.apiSettings?.categories?.find(c => c.name.toLowerCase().replace(/\s+/g, '-') === id);
    if (!cat || !Array.isArray(cat.items)) return;

    const submenu = document.createElement('div');
    submenu.className = 'children-links';

    cat.items.forEach((item, i) => {
      const itemId = `${id}-item-${i}`;
      const a = document.createElement('a');
      a.className = 'side-nav-link child-link';
